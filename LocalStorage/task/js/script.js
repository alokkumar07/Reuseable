...
//   if (task === "") {
//     new Swal({
//       text: "You need to enter a task name.",
//       icon: "error",
...
const payload = JSON.stringify({
  ...
  new Swal({
    title: "Task Created",
    text: `Your task "${payload}" has been created.`,
    icon: "success",
    // showConfirmButton: true,
  }).then(()=>{
  ...
  for (var key of keys) {
    ...
    const ui = `
      <tr class="border-b border-slate-200">
                    ...
                    <option value="sheduled" ${selectedStatus("sheduled",data.status)}>Sheduled</option>  

                    <option value="completed" ${selectedStatus("completed",data.status)}>Completed</option>
                    <option value="inprogress" ${selectedStatus("inprogress",data.status)}>In Progress</option>
                    <option value="cancelled" ${selectedStatus("cancelled",data.status)}>Cancelled</option>
                    <option value="postponed" ${selectedStatus("postponed",data.status)}>Postponed</option>
                    <option value="onhold" ${selectedStatus("onhold",data.status)}>On Hold</option>

                    <option value="notstarted" ${selectedStatus("notstarted")}>Not Started</option>
                    <option value="waiting" ${selectedStatus("waiting")}>Waiting</option>

                    </select>
                    </td>
                    <td>
                        <div class="flex items-center gap-3">
                            <button onclick="openDialogEdit('${data.task}','${key}')" class="bg-green-600 cursor-pointer w-6 h-6 rounded-full flex items-center justify-center hover:bg-indigo-700 transition-all duration-300 ease-in-out">
